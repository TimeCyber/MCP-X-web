...
export const QuickPrompts: React.FC<QuickPromptsProps> = ({ t, setPrompt, disabled, userEffects, onDeleteUserEffect }) => {
    ...
    const promptsArray = Array.isArray(builtInPrompts) ? builtInPrompts : [];
    ...
    return (
        <div className="relative" ref={wrapperRef}>
            <button
                ...
                title={t('quickPromptsAriaLabel') || '选择快捷效果'}
                className="flex-shrink-0 w-10 h-10 flex items-center justify-center text-white/70 rounded-full disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/10 hover:text-white transition-all duration-200"
            >
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M12 2.69l.13.04a7.9 7.9 0 0 1 5.92 6.61 8.27 8.27 0 0 1-1.77 6.13A8.52 8.52 0 0 1 12 21.31a8.52 8.52 0 0 1-4.28-5.83 8.27 8.27 0 0 1-1.77-6.13A7.9 7.9 0 0 1 11.87 2.73L12 2.69zM12 22v-1.16"/>
                    <path d="M9 19h6"/>
                </svg>
            </button>
            {isMenuOpen && (
                <div className="absolute bottom-full left-0 mb-3 w-80 max-h-96 overflow-y-auto bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-xl shadow-2xl p-2 flex flex-col gap-1 z-[100]">
                    <h4 className="px-2 pt-1 pb-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                        {t('myEffects.title') || '我的效果'}
                    </h4>
                    {userEffects.length > 0 ? (
                        ...
                        <p className="px-2 pb-2 text-xs text-gray-500">{t('myEffects.noEffects') || '暂无保存的效果'}</p>
                    )}
