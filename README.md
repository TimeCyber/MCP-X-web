...
MCP-X 是一个**企业级 AI 中台**，集成了 AI 对话、视频生成、图像编辑、前端应用构建等多种创作工具，为企业和开发者提供一站式的 AI 工作流解决方案。
...
### v0.1.4 Seedance 2.0 & 画布优化
- **🎬 Seedance 2.0 支持**：官网视频生成能力新增支持 **Seedance 2.0** 模型，画质与动态表现大幅提升
- **🖼️ 画布视频首帧优化**：修复图形工作台刷新后视频封面偶尔不显示的问题，改用 `onloadeddata` 事件替代 `onloadedmetadata`，增加超时保护，确保视频封面稳定渲染
- **🔒 全局 401 拦截**：所有 API 服务统一处理 Session 超时，自动跳转登录页
- **📋 粘贴 @标签自动关联**：输入框粘贴含 `@图片N` / `@视频N` 的文本时，自动匹配画布中的媒体元素；未匹配的标签显示为灰色

### v0.1.3 应用构建与预览
- **🍌 Nano Banana Pro 4K**：图像能力新增支持 **Nano Banana Pro 4K** 模型（高分辨率出图）
- **🔒 HTTPS 预览代理**：SSE 中解析「预览路径: /vite/{端口}」，预览地址为 `当前站点 origin + 路径`（与页面同协议），避免混合内容；本地存储键为 `dev_path_{appId}`
- **🖼 跨域预览编辑**：React/Vite dev 预览为跨域 iframe 时，通过 fetch + `srcdoc` 注入编辑器脚本；对 `about:srcdoc` 注入 URL 构造器补丁，缓解 react-router-dom 在 srcdoc 下的报错
- **📋 新建应用页**：页面下方展示「我的应用」列表（最近 12 条），可跳转构建页
- **✏️ 构建页体验**：对话输入单次上限 8000 字；「加载更多历史消息」向上追加时保持滚动位置；源代码 Tab 支持在线编辑后一键填入对话发给 AI

### v0.1.2 优化更新
- **📚 文档全面升级**：平台定位更新为「企业级 AI 中台」，文档站新增视频生成指南，补充图生视频、参考图生视频、首尾帧插值、图生图、蒙版重绘等完整 API 文档，新增 Base URL 说明（`https://www.mcp-x.com/prod-api`）及模型列表接口文档。文档结构同步适配 **OpenClaw（龙虾）**，让 OpenClaw 可以直接读懂并接入 MCP-X 的全部能力
- **🔐 登录页全面重设计**：玻璃拟态卡片、动态背景光晕、密码显示切换、输入框聚焦动效，视觉体验大幅提升
- **🎬 图生视频增强**：图形工作台选中图片后生成视频，自动将选中图片作为起始帧和参考图传入后端
...
支持国内主流大模型：千问、即梦、可灵、海螺、**Seedance 2.0** 等，国外主流大模型：Veo3，Runway等。
...
- **实时预览**：生成代码即时预览；生产环境可通过 Nginx 将 dev 服务代理为 `/vite/{端口}`，前端按 SSE「预览路径」拼接同源 HTTPS 地址
- **可视化编辑**：点击页面元素精确修改（跨域 dev 预览时通过注入脚本增强兼容性）
- **一键部署**：支持云端部署
- **代码下载**：完整代码包下载

### 4. AI 图像编辑器
支持国内主流大模型：千问、Seed 等，国外主流大模型：Nano Banana、**Nano Banana Pro 4K**。
- **文生图**：根据文字描述生成图像
- **图生图**：基于参考图像生成新图像
- **局部编辑**：蒙版支持局部区域编辑
- **多模型选择**：支持多种图像生成模型
...
⚠️ **重要提醒**：该项目已在相关部门注册软著，MCP-X已注册商标，请尊重知识产权。
